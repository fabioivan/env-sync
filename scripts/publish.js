...
async function main() {
  ...
  try {
    ...
    // Verifica se existe CHANGELOG
    const changelogPath = path.join(__dirname, '../CHANGELOG.md');
    if (fs.existsSync(changelogPath)) {
      console.log(chalk.cyan('\n📋 CHANGELOG.md encontrado'));
      const showChangelog = await question('Deseja visualizar as mudanças recentes? (s/n): ');

      if (showChangelog.toLowerCase().startsWith('s')) {
        try {
          const changelog = fs.readFileSync(changelogPath, 'utf8');
          const recentChanges = changelog.split('\n').slice(0, 30).join('\n');
          console.log(chalk.gray('\n' + '='.repeat(60)));
          console.log(chalk.yellow('📋 CHANGELOG (primeiras 30 linhas):'));
          console.log(chalk.gray('='.repeat(60)));
          console.log(chalk.white(recentChanges));
          console.log(chalk.gray('='.repeat(60)));
        } catch (error) {
          console.log(chalk.red('❌ Erro ao ler CHANGELOG.md'));
        }
      }
    }
    ...
    console.log(chalk.cyan(`1) patch (${currentVersion} → patch) - Correções de bugs, melhorias menores`));
    console.log(chalk.cyan(`2) minor (${currentVersion} → minor) - Novas funcionalidades, compatível`));
    console.log(chalk.cyan(`3) major (${currentVersion} → major) - Mudanças incompatíveis, breaking changes`));
